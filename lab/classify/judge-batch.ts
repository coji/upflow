 ...
 *
 * Gemini Batch Prediction API で PR を一括分類する。50% コスト削減＋レート制限回避。
 * 通常は judge.ts（逐次版）を使い、大量データ投入時にこちらを使う。
 *
 * 成功時は golden.json を丸ごと上書きする（追記マージしない）。
 * 失敗時は golden.json を更新しない（前回の結果がそのまま残る）。
 *
 * Usage:
 *   pnpm tsx lab/classify/judge-batch.ts [--model gemini-3.1-pro-preview] [--file xl_all.json]
 *
 * 全ファイルまとめて実行:
 ...
 *   pnpm tsx lab/classify/judge-batch.ts --resume batches/xxxx  # ジョブ名を直接指定
 *
 * ジョブの状態確認:
