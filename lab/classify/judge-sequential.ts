/**
 * Golden set ジャッジスクリプト（逐次版）
 *
 * Batch API がスタックする場合の代替手段。
 * 1件ずつ generateContent で分類し、1件完了ごとに golden.json に追記保存する。
 * Ctrl+C で途中停止しても、完了分は golden.json に残る。
 *
 * Usage:
 *   pnpm tsx lab/classify/judge-sequential.ts [--model gemini-3.1-pro-preview] [--file s_sample.json]
 *
 * 全ファイルまとめて実行:
 *   pnpm tsx lab/classify/judge-sequential.ts
 *
 * 途中から再開（golden.json に既にあるエントリをスキップ）:
 *   pnpm tsx lab/classify/judge-sequential.ts --continue
 */
