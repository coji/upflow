...
export const loader = async ({ request, params }: Route.LoaderArgs) => {
  ...
  if (refresh) {
    ...
    clearAllCache()
    throw redirect(
      `/admin/${params.organization}/repositories/add?${searchParams.toString()}`,
    )
  }

  const integration = await getIntegration(params.organization)
  if (!integration) {
...
export const action = async ({ request, params }: Route.ActionArgs) => {
  const integraiton = await getIntegration(params.organization)
  if (!integraiton) {
    throw new Error('integration not created')
  }
  ...
  try {
    await addRepository(params.organization, {
