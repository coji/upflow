...
describe('GitHub API URL helpers', () => {
  ...
  test('uses api.github.com when env is unset', () => {
    vi.unstubAllEnvs()
  ...
  test('uses GITHUB_API_BASE_URL without trailing slashes', () => {
    vi.stubEnv('GITHUB_API_BASE_URL', 'http://localhost:4001/api/v3///')

    expect(getGithubApiBaseUrl()).toBe('http://localhost:4001/api/v3')
    expect(githubApiUrl('search/repositories')).toBe(
      'http://localhost:4001/api/v3/search/repositories',
    )
  })
})
