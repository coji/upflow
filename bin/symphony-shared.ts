...
export interface RunOptions {
  ...
  /**
   * Environment for the spawned process. When omitted the child
   * inherits the parent's process.env. Set this to inject values like
   * issue titles via env vars (`"$ISSUE_TITLE"`) instead of building
   * shell-quoted arg strings, which is brittle for unicode / quotes.
   */
...
/**
 * takt creates one branch per workflow run with this naming pattern.
 * The numeric suffix is takt's own internal id, not stable across
...
export function formatDeliveryCommitMessage(
  ...
  return [
    issueTitle,
    '',
    `Generated by symphony pipeline for issue #${issueNumber}.`,
    '',
    `Closes #${issueNumber}`,
    '',
    'Co-Authored-By: Symphony <noreply@symphony.local>',
  ].join('\n')
}

export function formatDeliveryPrBody(args: {
  issueNumber: number
  implementReport: string
  superviseReport: string
}): string {
  const implementSummary =
    extractMarkdownSection(args.implementReport, 'Summary') ||
    '(no Summary section in implement-report.md)'
  const superviseSummary =
    extractMarkdownSection(args.superviseReport, 'Summary') ||
    '(no Summary section in supervise-report.md)'
  return [
    `🤖 Generated by Symphony pipeline for issue #${args.issueNumber}.`,
    '',
    '## Summary',
    '',
    implementSummary,
    ...
    '## Test plan',
    '',
    '- [x] takt acceptance step approved the diff',
    '- [x] takt simplify step ran without regression',
    '- [x] takt supervise judgment: COMPLETE',
    '- [ ] human reviewer confirms the diff matches the issue spec',
    '',
