...
describe('formatDeliveryCommitMessage', () => {
  it('puts the issue title on the first line and includes Closes #N', () => {
    const m = formatDeliveryCommitMessage(
      413,
      'chore: standardize Upflow branding',
    )
    expect(m.split('\n')[0]).toBe('chore: standardize Upflow branding')
...
describe('formatDeliveryPrBody', () => {
  const implementReport = [
    ...
    '## Summary',
    '',
    'Standardized product name to Upflow across 4 files.',
    '',
    '## Files Changed',
    '',
    '- README.md',
