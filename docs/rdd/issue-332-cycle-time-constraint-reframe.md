...
**前提**: 本 RDD は #429 (PR 種別分類、merged) で導入された `pull_requests.pr_type` を活用し、**`pr_type='normal'` を既定の集計対象**とする。release / template-merge / dependency は cycle time 計測の意味が成立しない (即マージ前提・他ブランチ commit・自動更新)、または既に `excludeBots` で除外されている。これらを既定で除外することで、以下の症状診断は「本物の人間作業の PR」のみに対して行われる。

複数の実テナントで計測したところ、以下が **テナント横断で共通** していた (詳細は内部調査参照。数値は #429 merge 前の集計だが、normal 種別を既定母集団に切り替えても本質的な傾向は変わらない):
...
- **`pull_requests.pr_type` カラムが #429 で永続化済み** (`db/tenant.sql`、4 値: release / template-merge / dependency / normal)。本 RDD では既定で `pr_type='normal'` のみを集計対象とし、release / template-merge / dependency を含めるトグルを別途用意する
...
1. **症状層 (主表示)**: 統計用語を使わず、普通のプログラマーが見て直感で分かる形にする
   - **全体感カード**: 時間バケット (1h / 1d / 3d / 1w / 1m+) ごとの件数を棒で見せる。「ほぼ全部 1 日以内、3 件だけ 1 ヶ月超」のような形が一目
   - **長い PR カード (Gantt 風)**: 個別の長い PR を長い順に棒表示 (上位 30 件)。1 行 = 1 PR、棒の長さ = cycle time。クリックで popover を開く
   - **特徴別分布カード**: PR size / 作成曜日 / 月 / author 別の偏りを件数で表示
...
3. **Insights 層 (仮説提示)**: **件数ベース** で症状を述べる + 仮説 + 確認の問い。例: 「**3 件の PR が 1 ヶ月超**。3 件とも XL サイズ。機能分割困難 / レビュー回避戦略 / 設計結合度のどれが要因か個別 PR で確認推奨」。「上位 10%」「全体の 70%」のような統計表現は使わない
...
- ユーザーは画面を開いたとき、**主表示として以下 3 つ** を確認できる
  - (a) **全体感カード**: 期間内マージ PR の長さの分布を時間バケット (1h / 1d / 3d / 1w / 1m+) の件数棒で見せる。「ほぼ全部 1 日以内、3 件だけ 1 ヶ月超」のような形が一目で分かる
  - (b) **長い PR カード (Gantt 風)**: 個別 PR を長い順に並べた棒表示 (上位 30 件)。1 行 = 1 PR、棒の長さ = cycle time
  - (c) **特徴別分布カード**: PR size、作成曜日、月、author の偏りを件数表示
- ユーザーは **PR 種別フィルタ** で `release` / `template-merge` / `dependency` を個別にトグルで含めることができる (既定: いずれも除外、`pr_type='normal'` のみ集計対象)
- ユーザーは **bot PR の除外/含む** をトグルで切り替えられる (既定: 除外、現状の挙動を維持。`pr_type` フィルタとは独立に AND で適用される)
...
- ユーザーは Insights で **件数ベースで述べた症状 + 仮説 + 確認すべき問い** を含む文を読める (例: 「3 件の PR が 1 ヶ月超。3 件とも XL サイズ。機能分割困難 / レビュー回避戦略 / 設計結合度のどれが要因か個別 PR で確認推奨」)
- **統計用語 (「上位 N%」「中央値」「p90」「全体の M%」等) は表示しない**。すべて件数とバケット名で表現
...
- `app/routes/$orgSlug/analysis/cycle-time/+functions/queries.server.ts:10-65`
  ...
  - **`pr_type` フィルタを追加** (既定 `pr_type='normal'`、URL param で `release` / `template-merge` / `dependency` を個別に含めるトグルに対応)
...
- `app/routes/$orgSlug/analysis/cycle-time/+functions/aggregate.ts`
  - **時間バケット集計関数**を追加 (1h / 1d / 3d / 1w / 1m+ の境界で件数を返す)
  - **長い PR の上位 N 件 (既定 30) を返す関数**を追加 (cycle time 降順、popover 用の最小フィールドを含む)
  - PR size / 曜日 / 月 / author 別の **件数集計関数** を追加 (バケット内件数を返す。比率や率は出さない)
  - 仮説ヒント生成関数を追加 (個別 PR の特徴から「XL サイズ」「金曜作成」「reviewer 指名なし」等のラベルを返す)
  - 新 Insights 生成関数を追加 (件数ベースで症状を述べ、仮説と確認の問いを返す)
- `app/routes/$orgSlug/analysis/cycle-time/index.tsx:80-235`
  - URL param `includeRelease` / `includeTemplateMerge` / `includeDependency` / `excludeBots` / `excludeNoReviewMerges` の解釈を loader / clientLoader に追加
...
- `app/routes/$orgSlug/analysis/cycle-time/+components/` に追加
  - **全体感カード**: 時間バケット件数を横棒で表示 (Recharts の BarChart / 自作 SVG どちらでも可)
  - **長い PR カード**: 個別 PR を Gantt 風の横棒で表示。クリックで popover を開く
  - **特徴別分布カード**: PR size / 曜日 / 月 / author の件数表示
  - **仮説形式の Insights カード**: 件数ベースの症状 + 仮説 + 問い
...
- `app/routes/$orgSlug/analysis/cycle-time/+functions/aggregate.test.ts` に新集計関数の代表シナリオを追加 (時間バケット件数集計、長い順上位 N 件抽出、特徴別件数分布、レビューなしマージ分離、仮説ヒント生成、新 Insights)
...
```text
Cycle Time 診断
目的: 期間内マージ PR の長さを見て、遅い PR を個別に深掘りする。
注意: 中央値で「速い」と読める期間でも、長い PR は数件まぎれていることがある。
業界文脈: DORA 2025 ではレビュー時間 +441%、PR サイズ +51.3% の悪化が報告されている。

既定母集団: pr_type='normal' (人間作業の PR のみ)
フィルタ: [✓ bot を除外] [✓ レビューなしマージを除外] [+ release を含める] [+ template-merge を含める] [+ dependency を含める] [reset]
...
| 1 ヶ月以内 ▇                                 12 件             |
| 1 ヶ月超   ▏                                  3 件             |
| → ほとんど 1 日以内、3 件だけ 1 ヶ月超                          |
+---------------------------------------------------------------+

+---------------------------------------------------------------+
| 長い PR (上位 30 件、長い順)         クリックで popover        |
| #1234 [XL] ████████████████████████████████  32 日             |
| #1156 [XL] ████████████████████              20 日             |
| #1098 [L]  ██████████                        10 日             |
| ...                                                           |
...
+---------------------------------------------------------------+
| Insights (仮説 + 確認の問い、件数ベース)                       |
| 3 件の PR が 1 ヶ月超。3 件とも XL サイズ。                    |
| 機能分割困難 / レビュー回避戦略 / 設計結合度のどれが要因か     |
| 個別 PR で確認推奨。                                           |
| 加えて 1 週間超の PR の半数が金曜作成 (期間全体では金曜投入は  |
| 14%)。週末越えを誘発している可能性、個別 PR で確認推奨。       |
+---------------------------------------------------------------+
```

統計用語ゼロで、件数とバケット名で全部表現。「上位 N%」「中央値」「p90」「全体の M%」のような言葉は画面に出さない。
...
2. **query 層 + URL param**: `pr_type` フィルタの追加 (既定 `normal` のみ、include 系トグル URL param)、`excludeBots` / `excludeNoReviewMerges` の URL param 化、`pull_request_reviews` JOIN 追加、cache key 拡張
...
- [ ] 実装後、Cycle Time 画面の主表示で、期間内マージ PR の長さの **時間バケット (1h / 1d / 3d / 1w / 1m+) の件数** が棒で確認できる
- [ ] 実装後、Cycle Time 画面の主表示で、**個別の長い PR を長い順** (上位 30 件) に棒表示で確認できる
- [ ] 実装後、ユーザーは画面上で **PR size、作成曜日、月、author** のどれに長い PR が偏っているかを件数表示で識別できる
- [ ] 画面上に「上位 N%」「中央値」「p90」「全体の M%」のような **統計用語が表示されない** (件数とバケット名のみで表現)
- [ ] 実装後、PR 種別フィルタが操作でき、既定で `pr_type='normal'` のみ集計、トグル ON で `release` / `template-merge` / `dependency` を個別に含めた集計に切り替わる
- [ ] 実装後、PR 種別フィルタと bot 除外フィルタは独立に AND で適用される (例: `dependency` を含めるトグル ON でも、bot 除外 ON ならば bot author の dependency PR は除外されたまま)
...
- [ ] `app/routes/$orgSlug/analysis/cycle-time/+functions/aggregate.test.ts` に「時間バケット件数集計」「長い順上位 N 件抽出」「特徴別件数分布 (PR size, 曜日, 月)」「レビューなしマージ分離」「仮説ヒント生成」「新 Insights」の代表シナリオが追加され green
...
- **時間バケットの境界の妥当性**: 暫定 [1h / 1d / 3d / 1w / 1m+] はテナント横断で見やすい区切りとして選んだが、組織によって体感が違う (例: 1h は早すぎる、1m+ までで足りない、3d と 1w の間が空きすぎる)。Open Question 1 で扱う。境界の選択は「データの形を歪めない」ことが優先 (最頻バケットだけ太くなって他が見えなくなる選択は避ける)
- **統計用語ゼロの方針による限界**: 「上位 10%」「中央値」を出さないことで、統計に慣れた読み手には情報が薄く感じられる可能性がある。本 RDD は「普通のプログラマーが直感で分かる」を優先し、統計派向けの追加表示はスコープ外。必要なら別ビューで対応 (Open Questions では扱わない、別 RDD)
...
1. **時間バケットの境界と Gantt 風カードの上位 N 件**: 本 RDD は暫定で バケット境界 [1h / 1d / 3d / 1w / 1m+]、Gantt 上位 30 件を採用。組織やデータ規模によっては境界 (例: 1m+ のさらに上に 3m+ を追加、1h を 4h に変える) や件数 (50 件 / 100 件) を調整したい場合がある。テナント設定で持つかは Open Question 3 (フィルタ既定値) と一括で判断
...
3. **フィルタ既定値のテナント設定対応**: レビューなしマージ率がテナント間で 10% 台 〜 30% 台後半まで分布するため、既定値を `organizationSettings` 等で持つかどうか。`pr_type` 既定値 (現状: `normal` のみ) も同様にテナントによっては「全種別を含めた集計を見たい」要望が出る可能性。本 RDD は全テナント共通の「除外既定 + トグル切替」で開始するが、運用してから設定化が必要になる可能性あり (スキーマ変更を伴うため別 RDD で扱う)
