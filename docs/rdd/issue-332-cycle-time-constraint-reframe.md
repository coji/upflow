...
- [ ] RDD に `<!-- DRAFT: 人間レビュー前。受け入れ条件・代案・移行方針は要確認。 -->` が含まれている。
- [ ] RDD の「設計判断」に、結論、理由、採らなかった代案が 2 案以上含まれている。
- [ ] RDD 内で binding constraint の暫定定義が明文化されている。
- [ ] RDD 内に採用 UI 候補 A / B / E と ASCII モックが含まれている。
- [ ] RDD 内に移行戦略として「新 constraint view を主表示、旧 median view を sub-tab または collapsed section に降格」が明記されている。
- [ ] RDD 内に median と constraint の共存方針が明記されている。
- [ ] RDD 内にこの画面が「PR flow の制約候補を見つける画面」であり、「capacity planning」「reviewer 個別割当」「alerting」をしないことが明記されている。
