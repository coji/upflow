...
サイズ・hotfix・その他の sub-segmentation は本 RDD のスコープ外。必要に応じてダッシュボード層 (#426) が既存カラム (`additions` / `deletions` / `source_branch` / `complexity` / `changed_files` 等) から自由に派生させる。
...
種別判定は title / branch / author を入力に **決定論的** に行う。**評価ルール**: 行は上から順に評価し最初に該当した種別を採用する (priority order); 1 行内の主シグナル列は OR 評価で、いずれか 1 つでも該当すればその行が採用される。具体的な実装関数の構造・命名は実装フェーズで決める。
...
- **`pr_type` だけ永続化する理由**: 種別判定は title 正規表現 + branch + author の組み合わせで non-trivial。集計クエリで都度書くと保守負荷が高い。size / hotfix / 影響範囲などの sub-segmentation は単純な算術 / LIKE 式で済むため永続化不要 (dashboard 層が必要に応じて派生)
...
- **案 F: `pr_type` を 6 値で持つ (normal / release / template-merge / dependency / hotfix / large)** — 本 RDD の旧案 (PR #430 初版)。以下で却下:
  - hotfix と large は「本物 PR か否か」(分類軸) ではなく「本物 PR の中での性質」(sub-segmentation 軸) を表すため、1 列で 2 軸を兼ねるのは設計負債
  - XL 閾値や branch 規約のチューニング (例: 1000→800、`hotfix/*`→`hf/*`) のたびに既存データの再 migration が必要になる
  - そもそも cycle time 集計は median / p90 で outlier に頑健。large PR を filter する積極的理由が無い (むしろ大きな PR の遅さは #332 で見たい binding constraint)
  - 採用しない
...
CHECK 制約により実装バグでの不正値挿入を SQLite レベルで防ぐ。将来 enum 値を追加する際は declarative schema (`db/tenant.sql`) を更新し Atlas マイグレーションを再生成する。
...
- size / hotfix / 影響範囲などの sub-segmentation は本 RDD のスコープ外。dashboard 層 (#426) が必要に応じて既存カラムから派生
...
2. **本番デプロイ branch 名のテナント差**: 本 RDD は `target IN ('production', 'prod')` を固定値で持つ。観測 2 テナントは両方ともこの命名だが、`live`、`prd`、`prd-main` 等を使うテナントが将来現れた場合に `organizationSettings` で可変化するか (運用後に判断)
3. **不一致 warning の UI 表現**: 一般ユーザー向けの可視化が必要か (規約遵守を促すダッシュボード)、admin 専用で十分か
4. **stacked PR / sibling branch artifact 対策**: 実測 < 0.1% の頻度。`firstCommittedAt` が parent branch の commit を拾う問題。first-parent traversal では解決しない (sibling branch の linear history を遡るため)。完全解消には git history 解析が必要だが ROI が低い。defer して #426 ダッシュボード上で問題が顕在化した時点で別 RDD 化するか
5. **closed issue #428 の first-parent traversal**: 実測で対象 0 件 (本番デプロイフロー signal 追加後)。defer して、運用観測で必要が出たら別 issue 化
