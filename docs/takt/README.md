...
| workflow                | 用途                                                                                                           |
| ----------------------- | -------------------------------------------------------------------------------------------------------------- |
| `spec-implement-accept` | 仕様起草 → レビュー → 実装 → 受け入れ → simplify → 最終判定 のフル工程 (1 PR ぶんの実装)                       |
| `rdd-draft`             | issue を入力に `docs/rdd/issue-<N>-<slug>.md` の draft を 1 step で起こす。出力は DRAFT 扱いで人間レビュー必須 |
