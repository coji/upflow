...
**GitHub App** を使用する（OAuth App ではなく）。ユーザー認証とデータ取得の両方を担い、顧客 Org の private repo アクセスを installation token で行う。

#### 3a. GitHub App + ログイン + 自動紐付け
...
#### 3b. Installation Token によるデータ取得

1. DB スキーマ: `integrations` テーブルに `github_app_installation_id` カラム追加
2. GitHub App install 時の installation ID 保存エンドポイント
3. バッチ処理: installation token 生成 + 既存 token からのフォールバック
4. 管理画面: GitHub App install 状態の表示 + install リンク
