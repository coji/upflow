...
Turso 移行は Phase 4 (Worker 分散) の前提になる。
SQLite ファイルベースのままだと Worker 分散ができないため。
ただし Phase 1 (GitHub App + Webhook) や Phase 2-3 は Turso なしで進められる。

## 移行フェーズ

### Phase 1: GitHub App + Webhook (価値提供を最優先)

既存のパイプラインに影響ゼロで始められる。cron はそのまま動かし続ける。

**Phase 1a: GitHub App 基盤 + PAT 移行 + リポジトリ自動登録**

- GitHub App を作成 (マニフェストで権限宣言: contents:read, pull_requests:write, metadata:read)
- Express に `POST /api/github/webhook` を追加 (署名検証)
- `github_app_installations` テーブルで installation_id ↔ tenant_id を管理
- テナント設定画面に「GitHub App をインストール」ボタン
- Installation API でリポジトリ一覧を自動取得 → repositories テーブルに登録
- installation access token で API 呼び出し (PAT を置き換え)
- `installation_repositories` Webhook で追加/削除をリアルタイム同期
- 既存 PAT テナントは並行稼働、段階的に移行
- **効果**: オンボーディングの大幅簡素化、PAT の個人依存解消

```
[現状のオンボーディング]
org作成 → PAT発行(GitHub) → PAT入力(Upflow) → リポジトリ手動追加 × N → fetch開始

[GitHub App のオンボーディング]
org作成 → 「GitHub Appをインストール」→ GitHub側でリポジトリ選択 → 完了
```

**Phase 1b: PR サイズラベル**

- `pull_request.opened` / `pull_request.synchronize` でサイズ分類 → ラベル付け
- Webhook payload の additions + deletions だけ使うので DB 不要
- テナントごとに閾値設定可能 (organization_settings.size_thresholds)
- **効果**: 開発者が PR サイズを即座に把握、スループット改善のアクションが取りやすくなる

**Phase 1c: Webhook → 1 PR リアルタイム更新**

- Webhook イベントで該当 PR だけ fetch + transform (既存関数の組み合わせ)
- `fetcher.commits()`, `store.savePrData()`, `buildPullRequests()` は既に PR 単位で呼べる
- Webhook handler 内で同期実行 (1 PR なので数秒、GitHub 10s タイムアウトに収まる)
- **効果**: ダッシュボードがリアルタイムに更新される

**Phase 1d: cron 頻度の引き下げ**

- Webhook がメインデータソースになったら cron は取りこぼし補完に役割変更
- 30 分 → 数時間に 1 回に変更
- cron 側も installation access token に切り替え (PAT フォールバック残す)
- **効果**: API コスト削減

### Phase 2: ローデータテーブル分割 + fetch 純化

- `github_raw_data` (1行 fat JSON) → リソース種別ごとのテーブルに分割
- fetch は「API → ローデータ保存」だけに徹する (残りの加工ロジックを追い出す)
...
### Phase 4: Turso 移行 + Worker 分散

- SQLite → Turso (libSQL) に移行
- ジョブキュー (BullMQ 等) 導入
- Webhook handler をキュー投入に切り替え (同期実行 → `queue.add()`)
...
[現状: Personal Access Token]
  ...
  リポジトリ: 手動追加 (owner/repo を入力)

[目標: GitHub App]
  App の秘密鍵 → JWT → installation access token (自動発行、1時間有効)
  Rate limit: 5000/hour (installation 単位)
  リポジトリ: installation 時に選択 → API で自動取得
  Webhook: App レベルで自動設定 (リポジトリ個別の設定不要)
```

**GitHub App installation access token の発行フロー**:

```
App 秘密鍵 (サーバー保持)
  → JWT 生成 (exp: 10分)
  → POST /app/installations/{id}/access_tokens
  → installation access token (1時間有効)
  → GitHub API 呼び出しに使用
```

トークンは短命なのでリフレッシュの仕組みが必要。
octokit の `createAppAuth` が自動的にハンドルしてくれる。

**リポジトリ管理の変化**:

```
[現状]
integrations (PAT) → repositories (手動登録)

[GitHub App]
github_app_installations (installation_id)
  → Installation API: GET /installation/repositories
  → repositories テーブルに自動同期
  → installation_repositories webhook で追加/削除をリアルタイム反映
```

ユーザーが GitHub 側でリポジトリのアクセスを変更すると、
`installation_repositories` Webhook で Upflow 側も自動更新される。

**移行時の注意**:

- PAT → GitHub App は段階的に移行。既存テナントの PAT を即座に無効化しない
- `integrations` テーブルの `method` カラムで `pat` / `github_app` を区別
- fetcher 生成時に method を見て適切なトークン取得方法を選択
- 新規テナントは GitHub App のみ、既存テナントは設定画面で移行を促す
