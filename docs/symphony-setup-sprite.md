# Symphony worker sprite — setup runbook (DEPRECATED)

> **このアプローチは廃止しました。** sprites.dev は 30 秒アイドルで sprite が完全 hibernate (CPU off) するため、D2 (outbound poll Service) 案が成立せず、Fly machine への移行を選んだ。新しい本番手順は [docs/symphony-setup.md](./symphony-setup.md) を参照。
>
> 本ファイルは試行錯誤の記録として残してある (showcase narrative 用)。
