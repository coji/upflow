...
│    → サーバーが nonce を生成して DB に保存                        │
│    → GitHub のインストール画面にリダイレクト（nonce を state に）  │
│ 3. 対象 org（acme-corp）を選択 → Install                        │
│    → GitHub が setup callback URL にリダイレクト（nonce 付き）   │
│    → サーバーが DB で nonce を検証・消費 → organizationId 特定   │
│    → GET /app/installations/:id で GitHub API 検証               │
│    → shared DB に link 保存 + shared DB の integrations.method を自動切替 │
│    → Settings 画面にリダイレクト                                 │
...
│    → サーバーが nonce を生成して DB に保存                        │
│    → nonce 付きインストール URL がクリップボードにコピーされる    │
...
│    → GitHub が setup callback URL にリダイレクト（nonce 付き）   │
│    → サーバーが DB で nonce を検証・消費 → organizationId 特定   │
│    → link 保存 + method 自動切替（セッション不要、nonce で認証） │
...
GitHub App の接続は **DB ベースの nonce** で安全に行われる:

```
1. ユーザーが「インストール」or「URL コピー」をクリック
2. サーバーが nonce（UUID）を生成し DB に保存（organizationId + expiresAt 付き）
3. GitHub インストール URL に nonce を state として付与:
   https://github.com/apps/upflow-team/installations/new?state=NONCE_UUID
4. GitHub が App インストール後に setup callback にリダイレクト:
   /api/github/setup?installation_id=XXX&state=NONCE_UUID
5. サーバーが DB で nonce を検証・消費 → organizationId を取得
6. GET /app/installations/:id で installation を GitHub API 検証
7. shared DB に link 保存 + shared DB の integrations.method を github_app に切替
```

**セッション不要**: nonce が DB で organizationId に紐づいているため、セッションなしでも tenant を特定できる。UUID は推測不能で、単回消費 + 有効期限で安全性を担保。

**Webhook** は接続の正本ではなく **状態更新**（deleted, suspend, unsuspend, repository selection 変更）に使う。

> **設計変更の理由**: 以前の設計では (1) org 名を手入力 → webhook マッチ、(2) セッションで tenant 特定、(3) 全 installation 列挙だった。(1) はタイプミスリスク、(2) は複数タブ・委託で誤リンク、(3) はテナント間情報漏えい。DB ベースの nonce で3つとも解決。
...
4. **DB ベースの nonce で安全にテナント特定**: セッション不要。UUID は推測不能、単回消費 + 有効期限で安全性を担保。全 installation 列挙による情報漏えいリスクなし
...
3. **全 call site を更新**（漏れなし）:
   ...
   | `app/routes/$orgSlug/settings/repositories/$repository/$pull/queries.server.ts`             | `getRepositoryWithIntegration` → `getRepository` に簡素化（integration は action 側で分離取得） |
...
2. **State トークンユーティリティ**: `app/libs/github-app-state.server.ts`
   - `generateInstallState(organizationId)`: nonce（UUID）を生成し DB に保存、nonce を返す
   - `consumeInstallState(nonce)`: DB で検証（未消費 + 未期限切れ）→ 消費済みにして `organizationId` を返す
...
3. **Webhook エンドポイント**: `app/routes/api.github.webhook.ts`
   ...
   - `installation.deleted`:
     ...
     - shared DB の `integrations.method` は `github_app` のまま → UI で「要再接続」と表示
   - `installation.suspend` / `unsuspend`:
     - shared DB の `integrations.app_suspended_at` を更新（`github_app_links.organization_id` で org を特定）
...
4. **Setup URL callback**: `app/routes/api.github.setup.ts`
   ...
   - **state 検証**: `consumeInstallState(nonce)` で DB 検証 → `organizationId` を取得（セッション不要）
   ...
   - **shared DB の `integrations.method` を `github_app` に自動切替**（リンク完了 = 認証切替）
...
5. **接続解除 action**: Settings UI から呼ばれる
   ...
   - shared DB の `integrations.method` を `token` に変更（PAT があれば即復帰）
   - `clearOrgCache`
...
| `app/routes/.../repositories/$repository/$pull/queries.server.ts`      | 2   | `getRepositoryWithIntegration` → `getRepository` に簡素化               |
