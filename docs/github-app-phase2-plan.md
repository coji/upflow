...
**純粋に additive。既存 PAT フローへの影響なし。**

1. `@octokit/auth-app` を `devDependencies` → `dependencies` に移動（`package.json`）

2. `db/shared.sql` — `github_app_links` テーブルを新規追加:
   ...
   - **shared DB に置く理由**: webhook で `installation_id` → tenant を O(1) で特定するため。全 tenant DB 走査が不要
   - `UNIQUE` 制約で同一 installation / 同一 GitHub org の重複リンクを DB レベルで防止
   - `deleted_at` による soft delete: `installation.deleted` 時に物理削除せず `deleted_at` を設定。「過去に接続されていたが失われた」（要再接続）と「一度も接続されていない」を区別可能

3. `db/tenant.sql` — integrations テーブルにカラム追加:

   ```sql
   app_suspended_at TEXT NULL                        -- サスペンド日時（webhook で更新）
   ```

   - `method` は「今どの認証方式を使うか」を表す。**リンク完了時に自動で `github_app` に切り替わる**
   - GitHub App の接続先情報（`installation_id`, `github_org` 等）は shared DB の `github_app_links` で管理

4. マイグレーション生成: `pnpm db:migrate`（shared + tenant 両方）

5. 型再生成: `pnpm db:apply && pnpm db:generate`
   - `app/services/type.ts`（shared DB 型）と `app/services/tenant-type.ts`（tenant DB 型）が自動更新

6. `app/routes/$orgSlug/settings/_index/+schema.ts`:
   - `method: z.enum(['token'])` → `z.enum(['token', 'github_app'])`

7. `batch/db/queries.ts` — `getTenantData` に `github_app_links` の JOIN を追加:
   - shared DB から `installationId`, `githubOrg`, `githubAccountId`, `appRepositorySelection` を取得（`deleted_at IS NULL` 条件付き）
   - tenant DB からは `method`, `appSuspendedAt` を取得

8. `db/seed.ts` — 既存 seed は `method: 'token'` のまま。`github_app_links` は seed 不要（接続時に動的に作成）
...
1. **新規**: `app/services/github-octokit.server.ts`
   ...
   - 呼び出し元では `github_app_links`（`deleted_at IS NULL`）の存在を事前チェックし、なければユーザー向けエラー（「GitHub App が未接続です」）に変換
...
3. **全 call site を更新**（漏れなし）:
   ...
   | `app/routes/$orgSlug/settings/repositories/$repository/$pull/queries.server.ts`             | `getRepositoryWithIntegration` に `method` を select 追加 + `github_app_links` JOIN |
...
- `method`, `privateToken`, `appSuspendedAt` → tenant DB `integrations`（認証設定・状態）
- `installationId`, `githubOrg`, `githubAccountId`, `appRepositorySelection`, `deletedAt` → shared DB `github_app_links`（接続先ルックアップ）

**クロス DB 整合性**:

- **正本**: shared DB `github_app_links` が接続状態の正本。tenant DB の `method` は従属
...
- **定期 reconciliation**: Phase 2 では不要（2テナント）。将来必要になったら `GET /app/installations` と `github_app_links` を突合するバッチを追加
...
| ファイル                                                              | PR  | 変更内容                                                                |
| --------------------------------------------------------------------- | --- | ----------------------------------------------------------------------- |
| `db/shared.sql`                                                       | 1   | `github_app_links` テーブル新規追加                                     |
| `db/tenant.sql`                                                       | 1   | `app_suspended_at` カラム追加                                           |
| `app/services/type.ts`                                                | 1   | 自動生成（shared DB 型）                                                |
| `app/services/tenant-type.ts`                                         | 1   | 自動生成（tenant DB 型）                                                |
| `app/routes/$orgSlug/settings/_index/+schema.ts`                      | 1   | method enum 拡張                                                        |
| `batch/db/queries.ts`                                                 | 1   | `github_app_links` JOIN 追加                                            |
| `db/seed.ts`                                                          | 1   | 型エラー対応（必要な場合）                                              |
