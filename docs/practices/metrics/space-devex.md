...
> **原典確認の状況 (2026-05 検証時点)**: ACM Queue の 2 本 (SPACE, DevEx) は本書執筆時に外部 fetch がブロックされ、原文 PDF を直接確認できなかった。本書の記述は WebSearch で得た複数の二次ソース (上記 InfoQ / DX 記事等) と専門書のオンライン要約に基づく。各 framework の 5 次元 / 3 次元の分類自体は広く一致しており大きな食い違いは確認していないが、引用時は二次ソース由来を明示する。原典は ACM Digital Library から購読アクセスで入手可能。
