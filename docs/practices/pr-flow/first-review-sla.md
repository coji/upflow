...
「PR を開いてから最初のレビューが返ってくるまで」を一定時間内に保つ運用ルール。業界目安は **LinearB 推奨で 12 時間、DORA Elite で 6 時間** (詳細は本文)。これを守れないチームは、PR が放置される文化が根付く。AI 時代に PR 数が増えると、放置 PR の山が顕在化しやすい。
...
- **LinearB 推奨: 12 時間以内** (一般的な High performer 水準)
- **DORA Elite 水準: 6 時間以内** (上位パフォーマンス)
- 24 時間 (1 営業日) を超えると詰まり始める

(出典: [Code Review Best Practices That Actually Scale | Augment Code](https://www.augmentcode.com/guides/code-review-best-practices-that-scale))

実テナントの計測例では「中央値は SLA 内に収まっているが、上位 10% は数日単位で外れている」というパターンが観測されており、中央値だけ見ると問題が見えない典型 (具体数値は issue #332 の内部調査参照)。
...
実テナントの計測でも、Pickup Time 中央値は SLA 内に収まる一方で、上位 10% (p90) は SLA を大きく超えるケースがある。中央値中心の表示を信じると「うちは速い」と誤読する典型例。
