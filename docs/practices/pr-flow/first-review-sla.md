...
業界の調査では **6 時間以内** が High performer の目安。8 時間 (1 営業日) を超えると詰まり始める。実テナントの計測例では「中央値は SLA 内に収まっているが、上位 10% は数日単位で外れている」というパターンが観測されており、中央値だけ見ると問題が見えない典型 (具体数値は issue #332 の内部調査参照)。
