...
CLAUDE.md は **全タスクに普遍的に適用される指示** だけを置く (Claude Code 公式ベストプラクティス: "as few instructions as possible")。新しいルールを追加する前に **「これは別ファイルにできないか」を必ず問う**。

- **CLAUDE.md に置く**: 全タスクに効く一般原則 (例: 命名規則、PR ワークフロー、コミュニケーション)
- **`docs/agent-rules/` に逃がす**: 特定ドメインの詳細ルール (例: 機密情報の扱い)。CLAUDE.md には 3 行以内のポインタのみ
- **`docs/` 直下に逃がす**: 設計ガイド・運用ノウハウ (例: `docs/guides/`, `docs/ops/`)
- **auto memory に書く**: セッションで学んだ訂正・好み・失敗事例 (CLAUDE.md は書き換えない)

自己改善ループ (auto memory の更新、将来の週次 routine 等) も本方針に従う。**CLAUDE.md を肥大化させる更新は受け入れない**。
...
6. **外部公開面に機密情報が混入していないか**
   - PR description / commit message / docs / README / .github/ に **顧客名・テナント slug・顧客由来の具体数値** を書いていないか
   - 抽象化 (「実テナントの計測では」「詳細は内部 issue 参照」) で代替する。詳細: [docs/agent-rules/confidentiality.md](./docs/agent-rules/confidentiality.md)
...
- コミット前にブランチを作成し、`git push -u origin <branch>` → `gh pr create` の流れを守る

### CI / GitHub Actions

- **third-party action は SHA pin** する。`uses: org/action@<full-40-char-sha> # vX.Y.Z` の形式で、コメントに version を残す。tag (`@v6` 等) は upstream の retag で silent に動作変更しうる供給チェーンリスクなので使わない
- 新しい action を追加するときは `gh api repos/{owner}/{repo}/git/refs/tags/{tag} --jq .object.sha` で対応 SHA を取得して pin する
- pin した SHA は `.github/dependabot.yml` の github-actions ecosystem 設定で月次 auto bump される (security advisory が出れば随時)
...
- **専門用語・造語は文中で説明する**: 英語の固いカタカナ語、初出のニッチな略語、その場で名付けた仕組みの呼び名（例: 提案中のツール名）を、定義抜きでそのまま使わない。初出の段階で平易な日本語の言い換えを 1 文添える。リポジトリで既に共有されている語（`takt`, `persona`, `facet`, `inventory`, `structural test` 等）は説明不要。
